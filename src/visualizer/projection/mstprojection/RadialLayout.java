...
public class RadialLayout {
    ...
    public ArrayList<RadialLayout.Node> execute(Pair[][] edges) {
    ...
    private void init(Pair[][] edges) {
        ...
        for (int i = 0; i < edges.length; i++) {
            nodes.add(new RadialLayout.Node(i));
        }
        ...
        for (int i = 0; i < edges.length; i++) {
            RadialLayout.Node parent = nodes.get(i);
            parent.children = new ArrayList<Node>();

            for (int j = 0; j < edges[i].length; j++) {
                RadialLayout.Node ch = nodes.get(edges[i][j].index);
                ch.parent = parent;
