...
public class Kmeans extends Clustering {
    ...
    public ArrayList<ArrayList<Integer>> execute(Dissimilarity diss, Matrix matrix) throws IOException {
        try {
            //Vari�vel auxiliar para armazenar as centr�ides antigas
            Matrix oldCentroids;
            ...
            while (isCentroidModified && numberIterations < 30) {
                ...
                for (int point = 0; point < matrix.getRowCount(); point++) {
                    int nearestCentroid = 0; //armazenar� a centr�ide que o ponto pertencer�
                    double distance1 = diss.calculate(matrix.getRow(point),
                            this.centroids.getRow(nearestCentroid));

                    //para cada cluster existente, verificar qual centr�ide o ponto
                    //est� mais pr�ximo
                ...
                for (int i = 0; i < this.clusters.size(); i++) {
                    //se o cluster for vazio, copia aleat�riamente algum ponto como
                    //centr�ide
