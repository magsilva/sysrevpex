...
public class Delaunay implements java.io.Serializable {
    ...
    public Pair[][] execute(double[][] points) {
        ...
        Logger.getLogger(this.getClass().getName()).log(Level.INFO,
                "Delaunay time: " + (finish - start) / 1000.0f + "s");
    ...
    void init(final double[] p) {
        ...
        if (xyBound == 0) {
            xyBound = 1;
        }  // to omit crash if ALL points are (0,0)
    ...
    void splitTriangle(final int f) {
        ...
        final int n0 = neighbor[fp],  v0 = face[fp++],  n1 = neighbor[fp],  v1 = face[fp++];
    ...
    void splitEdge(final int f, final int e) {
        checkFaceArray();
        int fp = 3 * f, c = 3 * numberOfFaces, f0 = fp + e, f1 = fp + (e + 1) % 3, f2 = fp + (e + 2) % 3,
                n = neighbor[f0], v0 = face[f0], v1 = face[f1], nf = neighbor[f2];
        ...
        if (n >= 0) {
            ...
            int np = 3 * n, ne = getNeighbor(n, f), n0 = np + ne, n1 = np + (ne + 1) % 3, n2 = np + (ne + 2) % 3,
                    nn = neighbor[n1], v2 = face[n0];
    ...
    boolean edgeIllegal(final int f, final int e) {
        ...
        if (hasExteriorPoints) {
            ...
            final int v0 = face[fp + e],  v1 = face[fp + (e + 1) % 3],  v2 = face[fp + (e + 2) % 3],  v = face[getNeighborPtr(n, f)];
    ...
    void seekSegment(final int v0, final int v1) {
        if (!seekAndMarkSegment(v0, v1)) {
            ...
            // addMidPoint
            addPoint((0.5f * (point[p0++] + point[p1++])), (0.5f * (point[p0] + point[p1])));
    ...
    void eatFace(final int f) {
        ...
        int fp = 3 * f;
        final int n0 = neighbor[fp],  n1 = neighbor[fp + 1],  n2 = neighbor[fp + 2];
        final boolean s0 = segment[fp++],  s1 = segment[fp++],  s2 = segment[fp++];
        if (!s0 && (n0 >= 0)) {
            if (!obsolete[n0]) {
                eatFace(n0);
            }
            ...
            for (int j = 0; j < currentCurveLength; j++) {
                ...
                if (j < currentCurveLength - 2) {
                    for (int k = j + 2; k < currentCurveLength; k++) {
                        ...
                        if (edgesCross(e0x0, e0y0, e0x1, e0y1, p[i][2 * k], p[i][2 * k + 1],
                                p[i][2 * ((k + 1) % currentCurveLength)], p[i][2 * ((k + 1) % currentCurveLength) + 1])) {
                ...
                if (i < numberOfBoundaryCurves - 1) {
                    ...
                    for (int k = 0; k < nextCurveLength; k++) {
                        if (edgesCross(e0x0, e0y0, e0x1, e0y1, p[i + 1][2 * k], p[i + 1][2 * k + 1],
                                p[i + 1][2 * ((k + 1) % nextCurveLength)], p[i + 1][2 * ((k + 1) % nextCurveLength) + 1])) {
                            throw new RuntimeException("boundary semengts must not intersect !");
                        }
                    }
                }
    ...
    boolean edgesCross(final double e0x0, final double e0y0, final double e0x1, final double e0y1,
        ...
        if ((ax * cy - ay * cx) * (ax * dy - ay * dx) >= 0) {
            return false;
        }   // edge e1 does NOT cross line through e0
        if ((bx * ey - by * ex) * (bx * fy - by * fx) >= 0) {
            return false;
        }   // edge e0 does NOT cross line through e1
