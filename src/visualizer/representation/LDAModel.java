...
import visualizer.topic.LDATopicExtractor;

public class LDARepresentation extends AbstractRepresentation {
    ...
    public SparseMatrix getMatrix(long[] ids, ProjectionData pdata) throws IOException {
        ...
        List<String> vocabulary = vectorMatrix.getAttributes();
        if (pdata != null) {
        ...
        for (int i = 0; i < vectorMatrix.getRowCount(); i++) {
            content = new StringBuilder(50);
            row = vectorMatrix.getRow(i);
        ...
        Matrix matrix = getMatrices(lda, pdata, ids);
        if (pdata != null) {
            pdata.setLDAMatrices(new LDATopicExtractor(lda.getAlphabet(), matrix, lda.getSortedWords()));
        }
